# Información Personal y Preferencias Profesionales

## Datos personales
- **Edad:** 27
- **Idiomas:** Inglés (C1)

## Contacto
- **Correo:** enri1998@hotmail.com
- **LinkedIn:** https://www.linkedin.com/in/kikemerino/

## Expectativas económicas
- **Salario bruto anual esperado:** 28.000 – 34.000 €
- **Tipo de contrato preferido:** No especificado

## Modelo de trabajo
- **Modalidad preferida:** Híbrido
- **Proporción híbrido (si aplica):** No especificado
- **Disponibilidad para viajar:** Depende de la ciudad
- **Disponibilidad para reubicarse:** Sí

## Disponibilidad
- **Fecha de incorporación:** 1-2 semanas
- **Situación laboral actual:** No estoy trabajando pero estoy estudiando y formandome con proyectos de IA como este

## Motivación y búsqueda
- **Razón principal para cambiar:** Un buen ambiente de trabajo, en el que pueda aprender y prosperar como data scientist
- **Tipo de empresa preferida:** No tengo preferencia
- **Sector de interés:** No especificado
- **Rol ideal:** AI engineer

## Valores y cultura
- **Valores que busca en una empresa:** Buen ambiente de trabajo, respeto, mejora continua
- **Cultura empresarial deseada:** No especificado

## Habilidades y perfil
- **Fortalezas blandas destacadas:** Aprendo rápido, persistente, constante, curioso
- **Áreas de mejora reconocidas:** Dificultad para prestar atención en temas que no interesan, dificultad para hablar en público
- **Stack o tecnologías preferidas:** Python, SQL, GenAI

## Intereses personales
- **Hobbies o aficiones:** Deporte, viajar
- **Proyectos personales relevantes:** No especificado

## Notas adicionales
No hay nada relevante mencionado.
# Información Personal y Preferencias

## Expectativas salariales
- Rango para posiciones 100% remotas en España: **28.000 € – 32.000 € brutos anuales**.
- Si la oferta implica mudanza o reubicación: **negociable** (subir el rango según condiciones y destino).
- Prioridades clave más allá del sueldo: **buen ambiente de trabajo** + empresa que valore de verdad la **mejora continua y el aprendizaje**.

## Preferencias de ambiente de trabajo
- Flexible con el tamaño y estructura del equipo (pequeño/autónomo o más estructurado).
- Cómodo con **dailies / stand-ups** y **feedback constante**, ya que es el estilo con el que llevo trabajando últimamente y me resulta productivo.

## Lo que más valoro en un equipo / compañeros
- **Actitud de apoyo mutuo** (ayudarse sin egoísmos).
- **Mantener la calma** bajo presión.
- **Paciencia** (especialmente en aprendizaje o debugging).
- **Buen ambiente** tanto formal (reuniones, procesos) como informal (charlas, confianza, risas).
- Experiencia positiva que me marcó: etapa en **NTT Data** → confianza mutua alta, sin miedo a preguntar a superiores ni compañeros, gente siempre disponible para resolver dudas.
- Experiencia negativa que evito repetir: entornos donde cada uno va a lo suyo, poca comunicación y te dejan “solo ante el peligro” → genera estrés, aislamiento y errores evitables.

## Tecnologías y habilidades

### Dominio sólido (nivel producción / puedo enseñar a otros)
- **Python** (muy sólido, uso diario en proyectos profesionales)
- **SQL** (consultas avanzadas, optimización, modelado de datos)

### En aprendizaje activo (2026)
- Todo lo relacionado con **Generative AI (GenAI)** y **Machine Learning** en general.
- Proyectos prácticos en curso: desarrollo de chatbots y aplicaciones con LLMs y RAG.

## Herramientas y hábitos que más valoro
- **Git**: versionado de los cambios, commits descriptivos, rama por cada nueva funcionalidad.  
  Beneficio clave: avanzar incrementalmente sin miedo, poder volver atrás si algo sale mal y evitar líos grandes.
- **Feedback constante:** tener feedback de los compañeros y superiores para ir encaminando tu trabajo sin que haya malentendidos por falta de comunicación

## Proyecto actual que más me motiva
Este mismo **chatbot personal** que estoy construyendo (CVer).  
Me apasiona todo el proceso: diseño de arquitectura, chunking, embeddings, retrieval, prompts, integración con Streamlit, iteración continua...  
Es un proyecto que combina programación práctica, experimentación con IA y un toque creativo/personal.  
Ojalá poder trabajar profesionalmente en algo similar (aplicaciones con LLMs, interfaces conversacionales, RAG a medida, etc.).

## Mayor reto técnico hasta ahora
Durante la universidad, trabajando como investigador: proyecto completo de **deep learning** en solitario.  
- Problema: predecir la **capa de nieve futura** en diferentes cuencas hidrográficas del mundo bajo varios escenarios climáticos.  
- Enfoque: red neuronal **autorregresiva con variables exógenas**.  
- Resultado: tendencia clara a **menos nieve** en el futuro → publicado en revista científica (SSRN).  
- Lección principal: al principio todo parece abrumador (especialmente solo), pero con **constancia, motivación y pasos progresivos** se puede lograr cualquier cosa.

## Resumen en 3 líneas
Ingeniero informático especializado en Python y SQL, con experiencia en backend, datos y ahora muy motivado con GenAI/ML.  
Valoro entornos colaborativos con apoyo mutuo, calma, paciencia y buena comunicación (como en mi etapa en NTT Data).  
Busco roles donde pueda seguir aprendiendo rápido, contribuir en proyectos interesantes y trabajar con equipos que se apoyen de verdad.